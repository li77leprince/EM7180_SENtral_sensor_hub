...
void setup() 
{
  ...
  sd_file.open("/sentral.fw", O_RDONLY);  
  ...
  Serial.println("");Serial.println("writing data to EEPROM");
  for (MSadd = 0; MSadd < 256; MSadd++) 
  {   // MS address byte, 0 to 255
    numbytes = sd_file.read(buffer, 128); // 128 bytes per page, 500 pages
    Serial.print("first two bytes: "); Serial.print("0x"); Serial.print(buffer[0], HEX);Serial.print(" "); Serial.print("0x"); Serial.println(buffer[1], HEX);
